class ffgt_gln_gw::etckeeper {
  ...
  # Ensure that we do not track the ffgt_gln_gw module
  file_line {
    'etckeeper_puppet':
       path => '/etc/.gitignore',
       line => 'puppet/modules/ffgt_gln_gw/';
    'etckeeper_dotfiles':
       path => '/etc/.gitignore',
       line => '.*';
...
define ffgt_gln_gw::etckeeper::ignore {
  if defined(Class['ffgt_gln_gw::etckeeper']) {
